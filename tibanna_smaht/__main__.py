...
class Subcommands(_Subcommands):
    ...
    def descriptions(self):
        ...
        desc['deploy_tiger'] = 'deploy tibanna tiger to AWS cloud (tiger is for SMaHT only)'
        return desc
        ...
        _args['deploy_tiger'] = \
            [{'flag': ["-s", "--suffix"],
              'help': "suffix (e.g. 'dev') to add to the end of the name of" +
                      "tibanna_tiger and AWS Lambda functions within the same usergroup"},
             {'flag': ["-g", "--usergroup"],
              'default': '',
...
def deploy_tiger(suffix=None, usergroup='', subnets=None, security_groups=None, env=None):
    """deploy tibanna tiger to AWS cloud (tiger is for SMaHT only)"""
    API().deploy_tiger(suffix=suffix, usergroup=usergroup, subnets=subnets,
...
def rerun_many(sfn=TIBANNA_DEFAULT_STEP_FUNCTION_NAME, stopdate='13Feb2018', stophour=13,
    ...
    rerun_many('tibanna_tiger-dev')
    rerun_many('tibanna_tiger', stopdate= '14Feb2018', stophour=14, stopminute=20)
    """
